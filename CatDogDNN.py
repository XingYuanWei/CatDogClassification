...
class DNN_Model(nn.Module):
    def __init__(self):
        ...
        # 全连接层1
        self.fc1 = nn.Linear(3 * 53 * 53, 1024)  # input:16*53*53, output:1024
        # 全连接层2
...
# 优化器
optimizer = optim.SGD(dnn_model.parameters(), lr=LEARNING_RATE, momentum=0.9)
...
for epoch in range(EPOCHS):
    ...
    for i, (imgs, labels) in enumerate(train_loader):
        ...
        optimizer.zero_grad()
        # 前向传播
        dnn_model = dnn_model.cuda()  # 这里要从cuda()中取得，不然前面都放在cuda后面放在cpu，会报错，报“不在同一个设备的错误"
        output = dnn_model(imgs)
        # 计算损失
